...
year_merge.drop(['item','user'], axis=1, inplace=True)
...
for i in itemid_2_index.values():
    ...
    table.append([
        i, year_merge.loc[item_id,'title'], year_merge.loc[item_id,'year'], " ".join(genre_agg.loc[item_id][0]), year_merge.loc[item_id,'inter_year']
    ]) #  
