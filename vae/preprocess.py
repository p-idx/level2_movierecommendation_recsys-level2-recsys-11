...
def filter_triplets(tp, min_uc=5, min_sc=0):
    if min_sc > 0:
        ...
        tp = tp[tp['item'].isin(itemcount.item[itemcount['size'] >= min_sc])]

    if min_uc > 0:
        usercount = get_count(tp, 'user')
        tp = tp[tp['user'].isin(usercount.user[usercount['size'] >= min_uc])]
...
def main():
    ...
    unique_uid = user_activity.user.values # 전체 user uid
    print("(BEFORE) unique_uid:",unique_uid)
    np.random.seed(98765)
    idx_perm = np.random.permutation(unique_uid.size) # 여기서 user id 섞여버림
    ...
    ## train_plays를 전체 데이터로 사용하고 그 중에서, 일정 아이템을 가려두고 학습해보자
